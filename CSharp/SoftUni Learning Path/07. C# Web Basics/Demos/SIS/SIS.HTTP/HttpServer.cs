...
#pragma warning restore CS4014 // Because this call is not awaited, execution of the current method continues before the call is completed
        ...
        private async Task ProcessClientAsync(TcpClient tcpClient)
        {
            ...
            {
                var errorResponce = new HttpResponse(
                    HttpResponseCode.InternalServerError,
                    Encoding.UTF8.GetBytes(ex.ToString()));
                errorResponce.Headers.Add(new Header("Content-Type", "texy/plain"));
                byte[] responseBytes = Encoding.UTF8.GetBytes(errorResponce.ToString());
                await networkStream.WriteAsync(responseBytes);
                await networkStream.WriteAsync(errorResponce.Body);
            }
        }
    }
}
